...
class DBSCAN private(
  ...
  private def toMinimumBoundingRectangle(vector: Vector): DBSCANRectangle = {
    ...
    DBSCANRectangle(x, Vectors.dense(x.toArray.map(f => f + minimumRectangleSize)))
  }

  private def corner(p: Vector): Vector =
    Vectors.dense(p.toArray.map(f => (shiftIfNegative(f) / minimumRectangleSize).intValue * minimumRectangleSize))
  ...
  //    Vectors.dense(p.toArray.map((f => {
  //      if (f < 0) {
  //        f - minimumRectangleSize
  //      }
  //      else f
  //    }
  //      )
